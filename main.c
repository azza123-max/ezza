...
#define HUB_SVC_UUID BT_UUID_DECLARE_128( \
    BT_UUID_128_ENCODE(0x12345678,0x1234,0x5678,0x1234,0x56789ABCDEF0ULL) \
)
#define HUB_CHAR_UUID BT_UUID_DECLARE_128( \
    BT_UUID_128_ENCODE(0x87654321,0x4321,0x8765,0x4321,0x0FEDCBA98765ULL) \
)

ssize_t hub_read(struct bt_conn *conn, const struct bt_gatt_attr *attr,
...
BT_GATT_SERVICE_DEFINE(hub_svc,
    ...
    BT_GATT_CHARACTERISTIC(HUB_CHAR_UUID,
                           BT_GATT_CHRC_READ,
                           BT_GATT_PERM_READ,
                           hub_read, NULL, NULL)
);
...
static void start_gatt_advertising(void)
{
    int err;
    const struct bt_data ad[] = {
        BT_DATA_BYTES(BT_DATA_FLAGS, (BT_LE_AD_GENERAL | BT_LE_AD_NO_BREDR)),
        /* advertise your 128-bit service so phone apps can filter it */
    ...
    printk(err ? "GATT adv failed (err %d)\n" : "GATT adv started\n", err);
}
